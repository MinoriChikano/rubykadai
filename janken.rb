...
class Player
  ...
  class Janken
    def pon(player_hand, enemy_hand)
      ...
      puts "相手の手は#{janken[enemy_hand]}です。" #（自分の手 - 相手の手 + 3）% 3
      # Playerクラスの戻り値とEnemyクラスの戻り値からじゃんけんするロジックを作成します。
      if (player_hand == 0 && enemy_hand == 0) || (player_hand == 1 && enemy_hand == 1) || (player_hand == 2 && enemy_hand == 2)  # Playerクラスの戻り値(player_hand)とEnemyクラスの戻り値(enemy_hand)の値が同じだった場合
        # 「あいこ」を出力します。
        puts "あいこ"
        true
